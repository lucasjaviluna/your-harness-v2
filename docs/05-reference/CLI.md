...
Los errores de comandos finalizan con un exit code estable según la taxonomía inferior; el código textual `error.code` permite distinguir la operación fallida sin analizar el texto humano. La salida humana continúa usando mensajes legibles. La CLI no mezcla logs de diagnóstico con stdout JSON; los consumidores deben tratar stdout como contrato de datos cuando usan `--json`.
