...
| High-level Change CLI facade | Implemented (approval slice) | `yh change inspect/status` expose SDD and HITM state; `yh change propose/review` persist and display the complete handoff snapshot; `yh change approve` records immutable stage decisions bound to the exact handoff version/digest. No command writes OpenSpec until `apply`, which remains pending. |
