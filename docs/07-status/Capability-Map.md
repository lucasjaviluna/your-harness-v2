...
| High-level Change CLI facade | Implemented (read-only slice) | `yh change inspect` exposes projection/provenance/digest and `yh change status` combines SDD state, governed lifecycle and stale HITM approvals. `propose`, `review`, `approve` and `apply` remain pending. |
