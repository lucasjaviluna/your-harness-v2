...
| High-level Change CLI facade | Implemented (propose/review slice) | `yh change inspect/status` expose SDD and HITM state; `yh change propose/review` persist and display the complete handoff snapshot without writing OpenSpec or approving stages. `approve` and `apply` remain pending. |
