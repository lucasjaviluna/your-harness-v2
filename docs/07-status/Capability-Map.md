...
| SDD provider and OpenSpec materialization | Implemented (scoped) | `SddProvider` reads neutral projections; `SddMaterializer` creates new draft Changes through HITM, environment guard, atomic staging and digest verification. Governed Change transitions and durable history are available; existing Changes, provider CLI/skill invocation and `openspec/specs/**` updates remain pending. |
