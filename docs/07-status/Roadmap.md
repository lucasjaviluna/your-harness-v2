...
13. ~~Enable one Pi tool under `ExecutionEnvironmentGuard`.~~ **Completed narrowly:** Pi exposes a guarded `read` tool when `workspace.read` is explicitly configured, validates every path and limits files to 256 KiB; broader tools remain gated.
