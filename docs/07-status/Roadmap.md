...
Current UX increment: `yh change inspect`, `yh change status`, `yh change propose`, `yh change review`, `yh change approve`, guarded `yh change apply` and read-only `yh change recover` are available; apply requires the full HITM chain, explicit confirmation and base-digest revalidation, then records a durable success or failure materialization audit event. The Apply policy defines `approved → apply-ready → applying → materialized`, with `applying → apply-failed → applying` for explicit retries. The recovery contract and provider digest classification are available; persistent HITM resolution of recovery remains pending.
