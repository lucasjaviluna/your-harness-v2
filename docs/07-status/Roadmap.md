...
Current UX increment: `yh change inspect`, `yh change status`, `yh change propose`, `yh change review`, `yh change approve` and guarded `yh change apply` are available; apply requires the full HITM chain, explicit confirmation and base-digest revalidation, then records a durable success or failure materialization audit event. The Apply policy defines `approved → apply-ready → applying → materialized`, with `applying → apply-failed → applying` for explicit retries; CLI apply persists every phase, accepts explicit idempotency/retry keys, and `change status` exposes the transition history. Remaining work: define crash recovery for an interrupted `applying` state.
