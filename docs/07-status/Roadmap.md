...
5. ~~Define the first guarded SDD materialization boundary.~~ **Completed (scoped):** draft Changes can be previewed, HITM-approved and materialized atomically through `SddMaterializer`; existing replacements require the current base digest, and external command mode is opt-in with an injected runner. Provider CLI/skill invocation, checklist synchronization and `openspec/specs/**` effects remain pending.
...
17. Complete the SDD Change operational lifecycle: **transition policy, durable history, HITM/digest gates, explicit stale-approval projection and guarded CLI facade implemented**; pending integration with broader Change edits, task/spec synchronization and provider-native lifecycle updates.
...
19. ~~Definir el límite arquitectónico del TaskOrchestrator como consumidor de contratos SDD, HITM, materialización, ejecución y verificación.~~ **Primer incremento implementado:** ADR-013 y `TaskOrchestrator` calculan un plan read-only desde un snapshot; faltan el adaptador que reúna el snapshot durable y la consulta CLI, sin automatizar efectos.
20. Componer el snapshot durable del TaskOrchestrator y exponer su consulta CLI estable.

Current UX increment: `yh change inspect`, `yh change status`, `yh change propose`, `yh change review`, `yh change approve`, guarded `yh change apply` and `yh change recover` are available; apply requires the full HITM chain, explicit confirmation and base-digest revalidation, then records a durable success or failure materialization audit event. The Apply policy defines `approved → apply-ready → applying → materialized`, with `applying → apply-failed → applying` for explicit retries. `change recover` classifies the provider read-only and, with explicit HITM decision, persists `recovery-required → materialized|apply-failed` plus a recovery audit; explicit recovery replay returns the existing audit and diagnostics expose the interrupted attempt timeline. Remaining work: provider-specific crash recovery diagnostics.

Release hardening increment: process-level E2E now covers the governed Change flow, Apply guardrail rejection, recovery outcomes (`apply-failed`, `materialized`, `recovery-required`), idempotent replay, explicit retry and the WorkItem HITM scope guard. `npm run verify:packed-install` now verifies clean installation from the packed artifact in an external workspace.

First release proposal: see [First-Release.md](./First-Release.md). The recommended v0.1.0 scope is a reproducible, governed and auditable Change CLI foundation; TaskOrchestrator, real MCP transport, general tool execution and workflow command/script execution remain outside that release.

Handoff contract increment: `ChangeDraftHandoff`, its Application use case, local immutable repository, CLI propose/review/approve/apply integration and `ChangeMaterializationAudit` are implemented. The reviewed snapshot remains immutable; idempotency returns the existing event for the same key, while retries require a new key and may link to a previous attempt.
