...
The local YAML config can select the default runtime, the read-only SDD provider, SDD traceability and the execution-environment policy. See [project configuration](../05-reference/Project-Configuration.md). Pi's read-only tool is enabled only when `workspace.read` is explicitly listed; other tools remain disabled.
