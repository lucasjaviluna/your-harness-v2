...
`ExecutionTrace` is the bridge from an execution to evidence. `Evidence` is now an
...
`VerificationReport` and its repositories are public contracts for a later evaluator.
They do not yet evaluate evidence automatically, persist durably, authorize completion,
or transition a WorkItem. The design remains specified in [ADR-009](../adr/ADR-009.md).
