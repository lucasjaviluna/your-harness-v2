...
La fachada visible incluye `yh change inspect <id>` y `yh change status <id>`, que son read-only sobre el proveedor y el estado HITM. `yh change status` también expone el estado Apply actual y su historial de transiciones, separado de las aprobaciones y de la proyección SDD. `yh change propose <id>` lee tres archivos de contenido, genera el preview mediante el materializer y persiste un `ChangeDraftHandoff`; `yh change review <id>` muestra exactamente ese snapshot; `yh change approve <id>` registra una decisión HITM inmutable ligada a la versión y digest del handoff vigente. `yh change apply <id>` consume únicamente un handoff con la cadena completa hasta Apply Readiness, exige confirmación explícita y revalida el digest antes de materializar; cada intento registra un `ChangeMaterializationAudit` inmutable con actor, estrategia, provenance, digest, resultado y error opcional. `attemptId`, `idempotencyKey` y `retryOfAttemptId` permiten distinguir solicitudes repetidas y retries explícitos. El handoff revisado no se sobrescribe; la policy operacional de Apply queda separada del lifecycle SDD.
