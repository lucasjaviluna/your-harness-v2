...
La fachada visible comenzó con `yh change inspect <id>` y `yh change status <id>`. Ambos comandos son read-only: el primero muestra la proyección neutral y su provenance; el segundo combina la proyección actual con el lifecycle gobernado y proyecta aprobaciones HITM obsoletas. Las futuras operaciones `propose`, `review`, `approve` y `apply` deberán consumir los casos de uso existentes sin exponer comandos internos del proveedor.
