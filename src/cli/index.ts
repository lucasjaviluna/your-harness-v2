...
providerCommand
  ...
  .action((name: string) => {
    ...
    console.log(
      chalk.green(`✓ Switching default provider to ${providerName}...`),
    );
  });
...
providerCommand
  ...
  .action(async (name?: string, options?: { prompt?: string }) => {
    ...
    try {
      ...
      console.log(
        chalk.gray(`Tokens: ${response.usage?.totalTokens ?? "N/A"}`),
      );
...
pluginCommand
  ...
  .action(async (path: string) => {
    try {
      ...
      console.log(
        chalk.green(
          `✓ Plugin '${plugin.manifest.name}' v${plugin.manifest.version} installed`,
        ),
      );
    } catch (error) {
...
pluginCommand
  ...
  .action(() => {
    ...
    for (const plugin of plugins) {
      ...
      console.log(
        `  ${statusIcon} ${chalk.bold(plugin.manifest.name)} v${plugin.manifest.version}`,
      );
...
skillCommand
  ...
  .action((options: { category?: string }) => {
    ...
    for (const skill of skills) {
      ...
      console.log(
        `  ${statusIcon} ${chalk.bold(skill.definition.name)} v${skill.definition.version}`,
      );
      console.log(chalk.gray(`    Category: ${skill.definition.category}`));
      console.log(
        chalk.gray(`    Status: ${skill.enabled ? "enabled" : "disabled"}`),
      );
      if (skill.definition.templates) {
        console.log(
          chalk.gray(
            `    Templates: ${skill.definition.templates.map((t) => t.name).join(", ")}`,
          ),
        );
      }
      console.log();
  ...
  .action(
    async (
      ...
      if (!agent) {
        ...
        console.log(
          chalk.gray(
            `Available agents: ${Object.keys(builtInAgents).join(", ")}`,
          ),
        );
        return;
      }
      ...
      console.log(
        chalk.gray(`Provider: ${options.provider ?? config.defaultProvider}`),
      );
      console.log();
        ...
        const result = await agentRunner.run(agent, objective, {
          ...
          onEvent: (event) => {
            switch (event.type) {
              case "step:start":
                console.log(
                  chalk.gray(`  Step ${(event.data as any).iteration + 1}...`),
                );
                break;
              case "tool:start":
              ...
              case "tool:complete":
                ...
                console.log(
                  chalk.green(`  ✓ Tool completed in ${toolStep.duration}ms`),
                );
                break;
              case "agent:error":
                console.log(
                  chalk.red(`  ✗ Error: ${(event.data as any).error}`),
                );
                break;
            }
          },
...
mcpCommand
  ...
  .action(async () => {
    try {
      ...
      console.log(
        chalk.gray("Send JSON-RPC requests via stdin. Press Ctrl+C to stop."),
      );
...
mcpCommand
  ...
  .action(() => {
    ...
    for (const server of configuredServers) {
      ...
      console.log(
        chalk.gray(`    Status: ${server.enabled ? "configured" : "disabled"}`),
      );
      console.log();
    }
  });
...
workflowCommand
  ...
  .action(async (name: string, options: { provider?: string }) => {
    ...
    if (!workflow) {
      ...
      console.log(
        chalk.gray(
          `Available workflows: ${Object.keys(builtInWorkflows).join(", ")}`,
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        console.log(
          chalk.gray(`Duration: ${(result.totalDuration / 1000).toFixed(1)}s`),
        );
        console.log(
          chalk.gray(
...
specCommand
  ...
  .action((file: string) => {
    ...
    console.log(
      chalk.yellow(`  Warnings: ${validationResult.summary.warnings}`),
    );
    ...
    if (validationResult.warnings.length > 0) {
      ...
      for (const warning of validationResult.warnings) {
        console.log(
          chalk.yellow(`  ⚠ [${warning.validationId}] ${warning.message}`),
        );
...
specCommand
  ...
  .action((file: string, options: { target?: string; output?: string }) => {
    ...
    const target = (options.target ??
      "typescript-types") as SpecGenerationTarget;
    ...
    if (genResult.success) {
      console.log(
        chalk.green(`✓ Generated ${genResult.files.length} files:\n`),
      );
  ...
  .action(() => {
    ...
    const targets: Array<{ name: SpecGenerationTarget; description: string }> =
      [
        ...
        {
          name: "custom",
          description: "Custom generation target (requires template)",
        },
      ];
...
workItemCommand
  ...
  .description("Create a persistent WorkItem")
  ...
  .action(async (workItemId: string, options: { title: string; intent: string; workspace: string }) => {
    try {
      ...
      if (await store.workItems.findById(id)) {
        throw new Error(`Work item '${workItemId}' already exists.`);
      }
      await store.workItems.save(
        new WorkItem(id, new IntentId(options.intent), new WorkItemTitle(options.title)),
      );
...
workItemCommand
  .command("bind <workItemId>")
  .description("Bind a WorkItem to the configured SDD source")
  ...
  .action(async (workItemId: string, options: {
    ...
    try {
      ...
      const id = new WorkItemId(workItemId);
      if (!(await store.workItems.findById(id))) {
        throw new Error(`Work item '${workItemId}' was not found in local operational state.`);
      }
...
workItemCommand
  ...
  .description("Execute a persistent work item through the selected runtime")
  ...
  .action(async (workItemId: string, options: { workspace: string; constraint?: string; runtime?: string }) => {
    ...
    try {
      ...
      if (!binding) {
        throw new Error(`Work item '${workItemId}' has no persistent SDD execution binding.`);
      }
      const sddProject = await projectEnvironment.sddProvider.readProject({
