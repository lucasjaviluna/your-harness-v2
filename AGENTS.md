...
`your-harness` (`yh`) — CLI-first AI dev harness. TypeScript, Node 22.19+, Commander.js, Zod, pino.
...
- **Tests are focused, not broad** — cubren configuración, contexto, Core smoke flows, runtime composition/boundary, execution environment, planificación read-only de TaskOrchestrator, read-only OpenSpec projection, SDD drift, operational bindings, governed Change lifecycle, in-process and process-level CLI execution, Evidence/Verification contracts/evaluator, HITM authorization, eligibility, guarded Pi tool integration and Apply recovery.
- **CI/CD mínimo implementado** — `.github/workflows/ci.yml` ejecuta instalación limpia, build, tests, lint, validación documental y empaquetado en Node 22.19/24; el runtime mínimo se alinea con `@earendil-works/pi-coding-agent`.
...
- **Security baseline is documented for the first release** — paths, capabilities, confirmations, digest checks, atomic materialization and versioned local persistence are reviewed in `docs/07-status/Security-Review.md`. Multiprocess locking, OS-level sandboxing for external runners and the complete write-command E2E matrix remain release-hardening work.
...
- **High-level Change CLI facade has started** — `yh change inspect`, `yh change status`, `yh change propose`, `yh change review`, `yh change approve`, scoped `yh change apply` and `yh change recover` expose the provider projection, durable handoff and staged HITM decisions while hiding internal OpenSpec commands. Propose/review/recover remain non-approving by default; apply requires the complete HITM chain, explicit confirmation and digest revalidation, then records immutable materialization audit events and Apply transition history. Recover can persist a materialized/apply-failed resolution only with explicit HITM and matching provider evidence; explicit idempotency/retry keys and recovery replay are supported.
...
- **Workspace installation is reproducible with npm** — `package-lock.json` is committed, workspace packages use semver dependencies and resolve as local links inside the monorepo, while their independent tarballs can be installed externally; `rimraf@^6` is declared at the root and packages. Verify with `npm ci` followed by `npm ls` and `npm run verify:packed-install`.
