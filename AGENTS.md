...
- **High-level Change CLI facade has started** — `yh change inspect`, `yh change status`, `yh change propose` and `yh change review` expose the provider projection and durable handoff while hiding internal OpenSpec commands. Propose/review remain non-approving and non-materializing; `approve` and `apply` remain pending.
